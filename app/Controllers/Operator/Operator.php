...
class operator extends BaseController
{
    ...
    {
        $this->m_operator = new OperatorModel();
    ...
    public function login()
    {
        ...
        $dataAkun = $this->m_operator->getData($username);
        ...
        if ($this->request->getMethod() == 'post') {
            ...
            if (!$this->validate($rules)) {
                ...
                session()->set($akun);
                return redirect()->to("operator/sukses")->withCookies();
                    
            }
        echo view("operator/v_login", $data);
    }

    function sukses()
    {
        return redirect()->to('operator/article');
        // print_r(session()->get());
    ...
    { 
        ...
        if ($this->request->getMethod() == 'post') {
            ...
            if (empty($err)) {
                $data = $this->m_operator->getData($username);
                if (empty($data)) {
            ...
            if (empty($err)) {
                ...
                $link = site_url("operator/resetpassword/?email=$email&token=$token");
                $attachment = "";
                $to = $email;
                ...
                $this->m_operator->updateData($dataUpdate);
            ...
            return redirect()->to("operator/lupapassword");
        }
        echo view("operator/v_lupapassword");
    }
    ...
    function resetpassword()
    {
        ...
        if ($email != '' and $token != '') {
            $dataAkun = $this->m_operator->getData($email); //<-- cek di tabel user
        ...
        if ($this->request->getMethod() == 'post') {
            ...
            if (!$this->validate($aturan)) {
                ...
                return redirect()->to('operator/login')->withCookies();
            }
        }

        echo view("operator/v_resetpassword");
    }
}
